...
function SettingsPage() {
    ...
    // Disable Save when server form values match current server config
    const portNum = parseInt(frontendPort, 10);
    const serverValuesUnchanged =
        !Number.isNaN(portNum) &&
        portNum === CONFIG.FRONTEND_PORT &&
        invertScroll === CONFIG.MOUSE_INVERT &&
    ...
    return (
        <div className="h-full overflow-y-auto w-full">
            <div className="p-6 pb-safe max-w-md mx-auto space-y-8 min-h-full">
                ...
                {/* Server settings: saved to server-config.json on Save */}
                <div className="space-y-6">
                    ...
                    <button
                        ...
                        disabled={serverValuesUnchanged}
                        onClick={() => {
                        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
                ...
                {/* Client settings: localStorage only, never sent to server */}
